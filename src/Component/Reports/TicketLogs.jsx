...
import { EmployeeLogDate } from '../Common/Common';
const TicketLogs = ({ allReportData, debouncedOnChange, exportDataToCSV, loading }) => {

    return (
        <>
            <main className="bg-white flex-1 p-3 overflow-hidden">
                <div className="flex flex-col">
                    <section className="flex-1 flex-col md:flex-row lg:flex-row sm:flex-row mx-2 pb-5 flex items-center justify-between">
                        <div className="font-bold text-xl flex items-center"> <Link to="/reports">
                            <div className="fa fa-angle-left mr-3">
                                <IoChevronBack />
                            </div>
                        </Link>Ticket Logs</div>
                        ...
                        <div className="inline-block min-w-full shadow-md rounded-lg overflow-auto ">
                            <table className="min-w-full leading-normal">
                                <thead className="bg-blue-lighter">
                                    <tr>
                                        <th className="px-5 py-3 text-left text-lg font-semibold text-gray-900 ">Ticket ID</th>
                                        <th className="px-5 py-3 text-left text-lg font-semibold text-gray-900 ">Emploee</th>
                                        <th className="px-5 py-3 text-left text-lg font-semibold text-gray-900 ">Added Date & Time</th>
                                        <th className="px-5 py-3 text-left text-lg font-semibold text-gray-900 ">Last Updated</th>
                                        <th className="px-5 py-3 text-left text-lg font-semibold text-gray-900 ">Status </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {loading && (
                                    ...
                                    {allReportData?.SearchData?.length > 0 && !loading && (
                                        allReportData?.SearchData?.map((item) => {
                                            return (
                                                <tr className="border-b border-gray-200 bg-white" key={item?.ticketID}>
                                                    <td className="px-5 py-3 text-lg">
                                                        <p className="text-blue whitespace-no-wrap">{item?.ticketID}</p>
                                                    </td>
                                                    <td className="px-5 py-3 text-lg">
                                                        <div className="flex items-center">
                                                            {/*<img className="rounded-full w-10 h-10 mr-2" src="dist/images/user-1.png" alt="" />*/}
                                                    ...
                                                    <td className="px-5 py-3 text-lg">
                                                        <p className="text-gray-900 whitespace-no-wrap">{EmployeeLogDate(item?.createdDate)}</p>
                                                    </td>
                                                    <td className="px-5 py-3 text-lg">
                                                        <p className="text-gray-900 whitespace-no-wrap">{EmployeeLogDate(item?.updatedDate)}</p>
                                                    </td>
                                                    <td className="px-5 py-3 text-lg">
                                                        {item?.status === "ReOpen" && (
